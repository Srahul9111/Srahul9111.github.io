...
export let finishGameDialog = document.getElementById("finish_game");
...
export function startBallAnimation(
  ...
  let animationLoop = function () {
    firedBubble.dom.style.left =
      parseFloat(firedBubble.dom.style.left) + xEveryFrame + "px";
    firedBubble.dom.style.top =
      parseFloat(firedBubble.dom.style.top) - yEveryFrame + "px";
  ...
  setTimeout(() => {
    firedBubble.dom.style.webkitTransform =
      "translate(" + distanceX + "px," + (-distanceY + spriteRadius) + "px)";
  ...
  let partialApplication = function () {
    for (let i = 0; i < orphanBubbles.length; i++) {
      ...
      bubbleDom.style.transition =
        "transform " + 1.2 + "s cubic-bezier(0.59,-0.05, 0.74, 0.05)";
      bubbleDom.style.transition =
        "-webkit-transform " + 0.8 + "s cubic-bezier(0.59,-0.05, 0.74, 0.05)";
    ...
    for (let j = 0; j < numOfCol * 2; j++) {
      ...
      if (bubble) {
        ...
        bubble.setCoords(
          ...
          top + board.getBoundingClientRect().top + bubbleRadius
        );
